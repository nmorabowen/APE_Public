...
class Quad2D:
    ...
    def __init__(self, elementTag, node_list, membrane, type='planeStress', samplingPoints=3, load_direction=None, eval_points=[0,0], nDof=2, uMMS=None):
        """
    ...
    def _initialize_MMS(self):
        if self.uMMS is not None:
            self.bMMS=self.MMS_solution_function()
    ...
    def MMS_solution_function(self):
        """_summary_
        ...
        epsilon = sp.Matrix([[sp.diff(uMMS[0], x)],
                             [sp.diff(uMMS[1], y)],
                             [sp.diff(uMMS[0], y) + sp.diff(uMMS[1], x)]])

        # Calculo de esfuerzos
        sigma = E * epsilon

        # Calculamos las fuerzas de cuerpo
        bMMS = sp.Matrix([[sp.diff(sigma[0], x) + sp.diff(sigma[2], y)],
                           [sp.diff(sigma[1], y) + sp.diff(sigma[2], x)]])
        
        return bMMS
    
    def MMS_evaluation(self, eval_points):
        """Given the uMMS and the bMMS we evaluate the solution at the given points
        ...
        bMMS=self.bMMS
        #print(bMMS)
        x_val, y_val = eval_points[0,0], eval_points[1,0]
        #print(x_val)
        #print(y_val)
        
        u_MMS_eval = uMMS.subs({x: x_val, y: y_val})
        b_MMS_eval = bMMS.subs({x: x_val, y: y_val})
        
        u_MMS_eval = np.array(u_MMS_eval).astype(np.float64)
        b_MMS_eval = np.array(b_MMS_eval).astype(np.float64)
        
        #print(b_MMS_eval)
        
        return u_MMS_eval, b_MMS_eval
    
    def _get_bMMS(self, eval_points):
        """ Body force vector calculation
        ...
        """
        b=self.MMS_evaluation(eval_points)
    ...
    def calculate_K0(self):
        ...
        for r, weight_r in zip(roots, weights):
            for s, weight_s in zip(roots, weights):
                ...
                if self.uMMS is not None:
                    eval_points=self.transform_to_physical(r,s)
                    _,b=self._get_bMMS(eval_points)
                else:
                    b=b_loadDirection
